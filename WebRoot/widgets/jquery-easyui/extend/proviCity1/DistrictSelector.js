 ...
 * @author 郝启敏
 **/
DistrictSelector = new function() {
	//存放所有的数据
    var a = [];
	
	/**
	 * 绑定值
	 * @param {Object} g select对象
	 * @param {Object} f 值即code
	 * @param {Object} flag false表示省市 true为省市区
	 */
    var selectvalue = function(g, f,flag) {
		//缓存中找此对象
        var e = a[f];
        if (e && e.children) {
			//省市只有一个子
			if(!flag && e.children.length == 1){
				 //e.children.forEach(function(i) {e = a[i];});
				 e=a[e.children[0]];				
			}
	...
	**/
    this.attach = function(d, h, g) {
        ...
        f.selectProv = function(j, i) {
            ...
            selectvalue(h, d.value,flag);
            if (typeof i == "undefined" || i == true) {
                if (h.value) {
                    this.selectCity(h.value,flag)
                }
            }
        };
		//选中城市
        ...
        f.getDivisionText = function(j) {
			...
			for(var i=0; i<sels.length;i++ ){
				...
				if(sel.selectedIndex>=0){
					txts.push(sel.options[sel.selectedIndex].text);
				}
			}
            return txts.join(j || "")
        };
...
//初始化数据
DistrictSelector.initData(DistrictSelector.pcadate);