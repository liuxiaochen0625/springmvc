 ...
 * function 
 * @author zlzhang
 * 2013-12-12下午02:34:17
 */
public class MyAccessDecisionManager implements AccessDecisionManager{
    private Logger logger = LoggerFactory.getLogger(getClass());
	@Override
	public void decide(Authentication authenticaiton, Object object,
			Collection<ConfigAttribute> configAttributes) throws AccessDeniedException,
			InsufficientAuthenticationException {
		//logger.info("授权判断。。。。");
		if(configAttributes == null){
			//logger.info("数据库拥有可查看此请求URl的权限集合为空！");
			return;
		}
		//所请求的资源拥有权限（一个资源对多个权限）
		...
		while (ite.hasNext()) {
			...
			for( GrantedAuthority ga:authenticaiton.getAuthorities()){
				//logger.info("权限匹配---权限1==="+needRole.trim()+";用户所拥有的权限！======"+ga.getAuthority().trim());
